...
import { abrirConta, entrarNaEquipe, parearTablet } from "./apoio";
 ...
 * toques não criam dois pedidos. Usa a mesa 1, aberta no E2E-01, reparada aqui: a abertura
 * é da mesa, e não do aparelho.
 */
test("E2E-27: pedido de cancelamento recusado, pedido de novo e aprovado", async ({ browser }) => {
  ...
  let conta = await abrirConta(tablet);
  ...
  // Dois toques não criam dois pedidos: o mesmo pedido de novo, pela rota, devolve o que existe.
  ...
  const pedido = equipe.getByRole("listitem").filter({ hasText: "Mesa 1 pede cancelamento" });
  ...
  conta = await abrirConta(tablet);
  await expect(conta).toContainText("R$ 62,00");
});
